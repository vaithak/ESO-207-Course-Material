...
{
    int bracketCount=0;
    if(str[0]==")" || isOperator(str[0]))
        return false;
    
    if(str[end]=="(" || isOperator(str[end]))
        return false;
    ...
    for(int i=0;i<=end;i++)
    {
        ...
        else if(str[i]==")")
        {
            ...
            if(i!=0 && (str[i-1]=="(" || isOperator(str[i-1])) )
                return false;  
        }
        ...
        {
            if(i!=0 && isOperand(str[i-1]))
                return false;
            
            if(i!=end && isOperand(str[i+1]))
                return false;
        }
        else
        {
